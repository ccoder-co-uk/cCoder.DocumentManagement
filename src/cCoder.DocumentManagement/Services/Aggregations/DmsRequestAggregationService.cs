...
internal partial class DmsRequestAggregationService(
    IDmsInstanceOperationsExposure dmsOperationsExposure,
    IStreamBroker streamBroker,
    ILoggingBroker log
) : IDmsRequestAggregationService
{
    ...
    private async ValueTask<DmsProcessingResponse> HandleDeleteRequestDmsProcessingRequestAsync(DmsProcessingRequest request)
    {
        ...
        await dmsOperationsExposure.DropAsync(
            path: path,
    ...
    private async ValueTask<DmsProcessingResponse> HandleGetRequestDmsProcessingRequestAsync(DmsProcessingRequest request)
    {
        ...
        DmsResult result =
            downloadPaths.Length > 0
                ? dmsOperationsExposure.GetFilesZipped(
                    paths: downloadPaths)
                : dmsOperationsExposure.Get(
                    path: path,
                    version: version,
                    search: search);
    ...
    private async ValueTask<DmsProcessingResponse> HandlePostRequestDmsProcessingRequestAsync(DmsProcessingRequest request)
    {
        ...
        if (query.ContainsKey(key: "copyTo"))
        {
            ...
            {
                await dmsOperationsExposure.CopyAsync(
        ...
        if (query.ContainsKey(key: "moveTo"))
        {
            ...
            if (!string.IsNullOrWhiteSpace(value: newPath))
            {
                await dmsOperationsExposure.MoveAsync(
                    oldPath: path.Split(separator: '?')[0],
                    newPath: newPath
                );
            }
    ...
    )
    {
        ...
        if (query.ContainsKey(key: "unpack"))
        {
            ...
            await dmsOperationsExposure.UnpackAsync(
        ...
        {
            await dmsOperationsExposure.SaveAsync(
                path: destinationPath.FullPath,
                content: content);
        }
