...
internal partial class DmsHttpRequestAggregationService(
    IDmsHttpProcessingService dmsHttpProcessingService,
    IDmsRequestOperationsExposure dmsOperationsExposure,
    IWebDavOperationsExposure webDavOperationsExposure
) : IDmsHttpRequestAggregationService
{
        ...
        TryCatch(operation: async () =>
        {
            ...
            if (IsWebDavRequestDmsProcessingRequest(request: request))
            {
                DmsProcessingSession session =
                    await webDavOperationsExposure.ProcessDmsProcessingSessionAsync(
            ...
            else
            {
                ...
                {
                    DmsProcessingSession session =
                        await dmsOperationsExposure.ProcessDmsProcessingSessionAsync(
                            dmsProcessingSession: new DmsProcessingSession
                            {
                                Request = request
                            });
