...
public partial class FileEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseFileUpdateEventAsync()
    {
        ...
        fileEventBrokerMock
            .Setup(expression: x => x.RaiseFileUpdateEventAsync(message: It.IsAny<EventMessage<DataFile>>()))
        ...
        fileEventBrokerMock.Verify(
            expression: x => x.RaiseFileUpdateEventAsync(message: It.IsAny<EventMessage<DataFile>>()),
            times: Times.Once
        );
