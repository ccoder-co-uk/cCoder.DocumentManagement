...
public partial class DmsInstanceServiceTests
{
    ...
    public async Task ShouldDelegateToBrokerWhenCopy()
    {
        ...
        dmsInstanceBrokerMock.Setup(expression: x => x.CopyAsync(oldPath, newPath)).Returns(value: ValueTask.CompletedTask);

        // When
        await dmsInstanceService.CopyAsync(oldPath: oldPath, newPath: newPath);

        // Then
        dmsInstanceBrokerMock.Verify(expression: x => x.CopyAsync(oldPath, newPath), times: Times.Once);
